> 원본 요구사항 보존 문서. 2026-09-09부터 커피는 사진 없이 정보형 목록으로 제공한다. [현재 커피 기능](coffee-features.md)을 우선한다. 현재 구현/작업 순서는 [서비스 기준](../service-design.md)과 [개발 가이드](../development.md)를 따른다. 아래 초기 목록은 실제 재고나 최신 가격이 아니며 자동 상품 수집을 의미하지 않는다.
