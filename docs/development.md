...
혼자 쓰고 혼자 커밋하는 개인 앱이므로 테스트는 느슨하게 간다. 커버리지를 목표로 삼지 않고, 변경에 맞는 것만 돌린다. 수위 기준은 [AGENTS.md](../AGENTS.md)의 `테스트 수위`가 단일 기준이다.

```sh
npm run lint        # 거의 모든 변경
npm run typecheck   # 거의 모든 변경
npm test            # 서버 규칙·계산·분류 룰을 건드렸을 때
npm run test:e2e    # 화면 흐름이 실제로 끊겼을 때만
npm run build       # 배포 전
```

통합/E2E는 로컬 `daily_test`를 초기화하며 동시에 실행하지 않는다. 통합 테스트는 DB와 사용자를 만들고 E2E도 별도 setup으로 초기화한다. 테스트 시나리오가 운영 데이터의 고정 병수에 의존하지 않게 한다. CI는 PostgreSQL 17 컨테이너에서 데스크톱 핵심 스모크 1건만 실행하며 Neon을 사용하지 않는다. 전체 데스크톱·모바일 회귀는 필요할 때 `npm run test:e2e`로 실행한다. 배포 DB는 PostgreSQL 18이다.

테스트를 새로 쓰지 않는 쪽이 기본이다. 예외는 되돌리기 어려운 영역과 실제로 겪은 버그뿐이다.

- 문구/CSS/화면 배치: 테스트를 쓰지 않는다. 타입·린트와 390px·1440px 실제 확인으로 끝낸다. 테스트 Chromium 결과를 실제 Safari 검증으로 표현하지 않는다.
- 저장/권한/금액, 재고, 인증: 되돌리기 어려우므로 기존 테스트를 유지한다. 다른 공간 접근·읽기 전용 scope·음수 재고·버전 충돌이 여기 해당한다. 새 기능이라고 해서 이 목록을 그대로 복제하지는 않는다.
- 자산 분류 룰: 룰을 고치면 `tests/assets.test.ts`가 먼저 깨져야 한다. 룰 한 줄이 전체 분류를 좌우해서 눈으로 못 잡는다.
- 실제로 겪은 버그: 같은 일이 반복되지 않게 한 건만 고정한다. 겪지 않은 상황을 미리 테스트하지 않는다.
- 낡은 테스트는 고치거나 지운다. 통과시키려고 구현을 비틀지 않는다.
...
현재 main push는 Vercel 운영 배포를 시작한다. push는 사용자가 직접 실행하고 에이전트는 커밋까지만 한다. 브랜치는 `feature/설명`을 사용하고 작업 요청의 배포 범위를 확인한다. 배포가 요청된 작업은 [배포 절차](deployment.md)에 따라 운영 DB의 호환 migration을 먼저 적용하고 Ready를 확인한다. 정적 문서의 예시나 옛 스킬 자체는 운영 변경 권한이 아니다.

## 다른 에이전트에게 전달할 작업 예시

> AGENTS.md와 docs/README.md의 순서대로 읽고 MONO의 `/요청한경로`를 추가해주세요. 기존 인증·공유 공간·MCP 규칙을 유지하고 모바일/PC 화면을 구현하세요. 기존 수정 사항을 보존하고, 필요한 migration·서버 검증과 문서를 함께 갱신하세요. 테스트는 AGENTS.md의 `테스트 수위`를 따르세요. 배포 범위: [로컬만 / 운영까지]. 기능과 완료 기준: [구체적 요구사항].
