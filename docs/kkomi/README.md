...
| 타입별 보기 · 배뭉침·통증 | 두 타입을 시간순 한 타임라인으로 합친다. 출혈은 성격이 달라 넣지 않고 출혈 탭에서만 본다. 요약은 타입별 한 칸씩(횟수·평균 간격·지속), 시간대별 차트는 두 타입 막대를 나란히 둔다. 행마다 타입 이름과 같은 타입끼리의 번호·간격을 표시하고, 기록 사이가 60분을 넘으면 "2시간 29분 동안 기록 없음"을 표시 |
