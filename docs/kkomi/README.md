...
| 타입별 보기 | 한 페이지 안에서 전체·배뭉침·통증·출혈 전환(기본 전체). 기간(최근 1시간·오늘·전체), 요약 3칸, 오늘 시간대별 횟수(1시간 4회 점선), 날짜별 세로 타임라인(시작·지속·번호·간격) |
| 타입별 보기 · 전체 | 세 타입을 시간순 한 타임라인으로 합친다. 요약은 타입별 한 칸씩(배뭉침·통증: 횟수·평균 간격·지속, 출혈: 확인·출혈·없음·사진 수). 시간대별 차트는 배뭉침·통증 막대를 나란히 두고 출혈은 축 아래 점(출혈 없음은 빈 점). 배뭉침·통증 행은 같은 타입끼리의 번호와 간격, 출혈 행은 양·색과 흐린 작은 사진. 기록 사이가 60분을 넘으면 "2시간 29분 동안 기록 없음"을 표시 |
| 출혈 목록 | 확인·출혈·사진 수 요약. 사진은 흐리게 보이고 한 번 누르면 선명, 다시 누르면 전체 화면 |

보기 상태는 URL에 남긴다: `?tab=type&kind=pain&range=all`. 전체 보기는 `kind`를 생략한다.
