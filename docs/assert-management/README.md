...
- KPI는 **총자산 카드가 왼쪽에서 두 줄을 차지하고 연평균 증가율·원화/달러 비율이 오른쪽에 쌓인다.** 세 칸을 같은 폭으로 두면 구성원 내역이 있는 총자산만 길어지고 옆 두 칸이 비어 보인다. 900px 이하에서는 총자산이 한 줄을 다 쓰고 나머지 둘이 아래에 나란히, 620px 이하에서는 한 줄씩 선다. 핵심 숫자는 보조 문구(`small`)가 아니라 큰 값으로 적는다.
- 총자산 카드는 금액·직전 대비 증감 아래에 **표시 범위의 총자산 추이 선**(축 없는 스파크라인)을 두고, 그 아래 구성원별 금액·비중·기준일·증감을 적는다. 추이 선이 남는 높이를 채우므로 구성원이 한 명이어도 카드가 비지 않는다. 연평균 증가율 KPI에는 **비율과 함께 그 구간의 실제 증감액과 시작→끝 금액**을 적는다. 비율만 있으면 "그래서 얼마 늘었나"를 못 읽는다. 원화/달러 KPI는 두 통화를 각각 비율·금액으로 나란히 적고, 통화가 없는 자산(금 등)은 막대와 한 줄 문구로만 보인다.
...
- **주식 보유 현황**은 `주식(kr_stock)`과 `해외 주식(foreign_equity)` 두 그룹만 카드로 보여준다. 최신 기간에 유효한 스냅샷의 원본 종목을 금액순 TOP 5로 적고, 그룹 머리말에 보유 종목 수와 수량 합계를, 각 종목에 그룹 내 비중과 수량을 적는다. 같은 종목명은 **구성원이 달라도 한 줄로 합친다**. 종목의 규모를 보는 표이지 사람별 보유를 보는 표가 아니다. 수량이 한 건도 기록되지 않은 그룹은 0이 아니라 표시하지 않는다. 카드의 **전체 보기**(그룹 이름이나 하단 버튼)는 같은 기준의 전체 종목과 그룹 내 비중을 팝업으로 연다. 자산 구성 도넛에서 `주식`·`해외 주식` 조각을 눌러도 원본 항목 서랍이 아니라 이 팝업이 열린다(자산 그룹 축일 때). 다른 조각과 추이·표의 셀은 기존대로 원본 항목 서랍을 연다. 서버 응답의 `stocks[].holdings`는 금액순 전체 목록이고 카드는 앞 5줄만 그린다.
